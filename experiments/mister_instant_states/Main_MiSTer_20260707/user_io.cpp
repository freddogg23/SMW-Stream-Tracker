...
static void *ss_slot_base[4] = {};
static int ss_enabled = 0;
static int ss_virtual_save_pending = 0;
static unsigned long ss_virtual_save_deadline = 0;
static int ss_virtual_restore_pending = 0;
static unsigned long ss_virtual_restore_deadline = 0;
static unsigned long ss_timer = 0;

#define SMW_VIRTUAL_STATE_ENABLE_FILE "/media/fat/SMWStreamTracker/enable_virtual_states"
#define SMW_VIRTUAL_LOAD_GUARD_MS 3000
#define SMW_VIRTUAL_SAVE_GUARD_MS 1000

static int ss_virtual_states_enabled()
{
	return is_snes() && access(SMW_VIRTUAL_STATE_ENABLE_FILE, F_OK) == 0;
}

static int ss_state_path(int slot, char *path, size_t path_size)
{
	if (slot < 1 || slot > 11 || !ss_sufx || !ss_name[0]) return 0;

	size_t suffix_offset = ss_sufx - ss_name;
	if (suffix_offset >= strlen(ss_name) || suffix_offset > INT_MAX) return 0;
...
static int ss_virtual_state_path(int slot, char *path, size_t path_size)
{
	return slot >= 5 && slot <= 11 && ss_state_path(slot, path, path_size);
}

static int ss_virtual_state_slot_for_key(uint16_t key)
{
	if (key >= KEY_F5 && key <= KEY_F10) return 5 + (key - KEY_F5);
	if (key == KEY_F11) return 11;
	return 0;
}

static void ss_restore_native_slot_four()
{
	if (!ss_slot_base[3]) return;

	char native_path[1024] = {};
	if (!ss_state_path(4, native_path, sizeof(native_path))) return;

	memset(ss_slot_base[3], 0, ss_size);
	fileTYPE f = {};
	if (FileOpen(&f, native_path, 1))
	{
		int ret = FileReadAdv(&f, ss_slot_base[3], ss_size);
		FileClose(&f);
		printf("SMW virtual state: restored native slot 4 (%d bytes) from %s\n", ret, native_path);
	}
	ss_cnt[3] = 0xFFFFFFFF;
	*((uint32_t *)ss_slot_base[3]) = 0xFFFFFFFF;
	__sync_synchronize();
}

static void ss_schedule_native_slot_four_restore(unsigned long delay_ms)
{
	ss_virtual_restore_pending = 1;
	ss_virtual_restore_deadline = GetTimer(delay_ms);
	ss_timer = 0;
}

static int ss_virtual_state_busy()
{
	return ss_virtual_save_pending || ss_virtual_restore_pending;
}

static void ss_pulse_snes_action(const char *action)
{
	uint32_t previous_slot = user_io_status_get("[52:51]");
	user_io_status_set("[52:51]", 3);
	usleep(20000);
	user_io_status_set(action, 1);
	usleep(20000);
	user_io_status_set(action, 0);
	user_io_status_set("[52:51]", previous_slot);
}

static int ss_virtual_state_load(int slot)
{
	if (!ss_virtual_states_enabled() || !ss_enabled || !ss_slot_base[3]) return 0;
	if (ss_virtual_state_busy())
	{
		Info("Please wait for the current virtual state", 1000);
		return 1;
	}

	char path[1024] = {};
	if (!ss_virtual_state_path(slot, path, sizeof(path))) return 0;

	fileTYPE f = {};
	if (!FileOpen(&f, path, 1))
	{
		char message[64];
		snprintf(message, sizeof(message), "Virtual state %d is empty", slot);
		Info(message, 1000);
		return 1;
	}

	__off64_t file_size = FileGetSize(&f);
	if (file_size < 12 || file_size > ss_size)
	{
		FileClose(&f);
		Info("Virtual state file is invalid", 1200);
		return 1;
	}
	...
	{
		FileClose(&f);
		Info("Not enough memory to load virtual state", 1200);
		return 1;
	}

	int bytes_read = FileReadAdv(&f, buffer, file_size);
	FileClose(&f);

	uint32_t *header = (uint32_t *)buffer;
	uint64_t expected_size = ((uint64_t)header[1] + 2) * 4;
	if (bytes_read != file_size || expected_size != (uint64_t)file_size || header[2] != 0x53454E53)
	{
		free(buffer);
		Info("Virtual state file is incompatible", 1200);
		return 1;
	}

	memset(ss_slot_base[3], 0, ss_size);
	memcpy(ss_slot_base[3], buffer, file_size);
	free(buffer);
	...
	*((uint32_t *)ss_slot_base[3]) = 0xFFFFFFFF;
	...
	ss_pulse_snes_action("[48]");
	// The SNES core consumes the shared slot asynchronously. Restoring slot 4
	// here used to overwrite that memory while a load was still in progress,
	// which could corrupt the running game. Keep the virtual state intact for a
	// generous guard interval and restore the native slot in process_ss().
	ss_schedule_native_slot_four_restore(SMW_VIRTUAL_LOAD_GUARD_MS);
	char message[64];
	snprintf(message, sizeof(message), "Loaded virtual state %d", slot);
	Info(message, 800);
	printf("SMW virtual state: loaded slot %d from %s\n", slot, path);
	return 1;
}

static int ss_virtual_state_save(int slot)
{
	if (!ss_virtual_states_enabled() || !ss_enabled || !ss_slot_base[3]) return 0;
	if (ss_virtual_state_busy())
	{
	...
	ss_virtual_save_pending = slot;
	ss_virtual_save_deadline = GetTimer(5000);
	ss_timer = 0;
	ss_pulse_snes_action("[47]");

	char message[64];
	snprintf(message, sizeof(message), "Saving virtual state %d", slot);
	Info(message, 800);
	printf("SMW virtual state: requested save for slot %d\n", slot);
	return 1;
}
...
int process_ss(const char *rom_name, int enable)
{
	...
	{
		ss_enabled = enable;
		ss_virtual_save_pending = 0;
		ss_virtual_save_deadline = 0;
		ss_virtual_restore_pending = 0;
		ss_virtual_restore_deadline = 0;
		...
		for (int i = 0; i < 4; i++)
		{
			if (!ss_slot_base[i]) ss_slot_base[i] = shmem_map(map_addr, len);
			if (!ss_slot_base[i])
			{
			...
			else
			{
				...
				if (FileExists(ss_name))
				{
					...
					{
						int ret = FileReadAdv(&f, ss_slot_base[i], len);
						FileClose(&f);
						printf("process_ss: read %d bytes from file: %s\n", ret, ss_name);
					}
				}
				*(uint32_t*)(ss_slot_base[i]) = 0xFFFFFFFF;
			}
	...
	if (ss_virtual_restore_pending && ss_virtual_restore_deadline && CheckTimer(ss_virtual_restore_deadline))
	{
		ss_virtual_restore_pending = 0;
		ss_virtual_restore_deadline = 0;
		ss_restore_native_slot_four();
		printf("SMW virtual state: native slot 4 restored after the safety guard\n");
	}

	if (ss_virtual_save_pending && ss_virtual_save_deadline && CheckTimer(ss_virtual_save_deadline))
	{
		ss_virtual_save_pending = 0;
		ss_virtual_save_deadline = 0;
		ss_schedule_native_slot_four_restore(SMW_VIRTUAL_SAVE_GUARD_MS);
		Info("Virtual state save timed out", 1200);
	}

	if (ss_timer && !CheckTimer(ss_timer)) return 0;
	ss_timer = GetTimer(ss_virtual_state_busy() ? 10 : 1000);
	...
	for (int i = 0; i < 4; i++)
	{
		if (ss_slot_base[i])
		{
			uint32_t curcnt = ((uint32_t*)(ss_slot_base[i]))[0];
			uint32_t size = ((uint32_t*)(ss_slot_base[i]))[1];
			...
			if (curcnt != ss_cnt[i])
			{
				...
				if (size > 0 && size <= ss_size)
				{
					...
					int is_virtual_capture = (i == 3 && ss_virtual_save_pending);
					if (!is_virtual_capture)
					{
						*ss_sufx = i + '1';
						if (FileOpenEx(&f, ss_name, O_CREAT | O_TRUNC | O_RDWR | O_SYNC))
						{
							int ret = FileWriteAdv(&f, ss_slot_base[i], size);
							FileClose(&f);
					...
					if (is_virtual_capture)
					{
						...
						int virtual_slot = ss_virtual_save_pending;
						ss_virtual_save_pending = 0;
						ss_virtual_save_deadline = 0;
						if (ss_virtual_state_path(virtual_slot, virtual_path, sizeof(virtual_path)) &&
							FileOpenEx(&f, virtual_path, O_CREAT | O_TRUNC | O_RDWR | O_SYNC))
						{
							int ret = FileWriteAdv(&f, ss_slot_base[i], size);
							FileClose(&f);
							char message[64];
							snprintf(message, sizeof(message), "Saved virtual state %d", virtual_slot);
							Info(message, 1000);
							printf("SMW virtual state: wrote %d bytes to %s\n", ret, virtual_path);
						}
						else
						{
							Info("Unable to save virtual state", 1200);
						}
						// Saving completes asynchronously too. Leave the captured bytes
						// untouched briefly before restoring the native slot 4 buffer.
						ss_schedule_native_slot_four_restore(SMW_VIRTUAL_SAVE_GUARD_MS);
					}
				}
			}
...
void user_io_kbd(uint16_t key, int press)
{
	...
	// Opt-in SNES virtual states 5-11 for SMW Stream Tracker. Native slots 1-4
	// remain normal; slot 4 is restored after a safety guard so the SNES core
	// can finish its asynchronous save/load before the shared bytes are touched.
	int virtual_slot = ss_virtual_state_slot_for_key(key);
	int virtual_shortcut = virtual_slot;

	if ((key == KEY_F4 || virtual_shortcut) && ss_virtual_states_enabled() && ss_virtual_state_busy())
	{
		if (press == 1) Info("Please wait for the current virtual state", 1000);
		return;
	}

	if (virtual_shortcut && ss_virtual_states_enabled())
	{
		if (press == 1)
		{
			if (get_key_mod() & (LALT | RALT)) ss_virtual_state_save(virtual_slot);
			else ss_virtual_state_load(virtual_slot);
		}
		return;
	}

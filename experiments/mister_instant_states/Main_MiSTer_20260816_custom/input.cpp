		...
		while (1)
		{
			...
			if ((pool[NUMDEV + 1].fd >= 0) && (pool[NUMDEV + 1].revents & POLLIN))
			{
				...
				if (len)
				{
					...
					else if (!strcmp(cmd, "smw_sram_save"))
					{
						const char *result = "not-snes\n";
						// The stock SNES core already exposes its safe Backup RAM
						// writer on status bit 13.  Pulse that same command for the
						// tracker without opening the OSD; the core keeps ownership of
						// the mounted save image and its normal block-transfer logic.
						if (is_snes())
						{
							uint32_t generation = user_io_smw_sram_write_generation();
							uint32_t observed_generation = generation;
							int stable_polls = 0;
							int write_seen = 0;
							user_io_status_set("[13]", 1);
							usleep(20000);
							user_io_status_set("[13]", 0);

							// The core sends the mounted save image back one block at a time.
							// Wait for real disk writes and for the stream to settle before
							// acknowledging the tracker. This turns the old fire-and-forget
							// response into a verified background save.
							for (int poll = 0; poll < 200; poll++)
							{
								usleep(20000);
								uint32_t current_generation = user_io_smw_sram_write_generation();
								...
								else if (write_seen && ++stable_polls >= 10)
								{
									break;
								}
							}

							if (write_seen && observed_generation != generation)
							{
								sync();
								printf("SMW SRAM save: Backup RAM flush verified\n");
								result = "saved\n";
							}
							else
							{
								printf("SMW SRAM save: core produced no save writes\n");
								result = "no-write\n";
							}
						}
						else
						{
							printf("SMW SRAM save: ignored outside the SNES core\n");
						}
						FILE *ack = fopen("/tmp/smw_sram_save_ack", "w");
						if (ack)
						{
							fputs(result, ack);
							fclose(ack);
